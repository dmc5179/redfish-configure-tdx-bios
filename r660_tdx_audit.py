...
# Settings that must be active before TDX-specific attributes become visible/settable
PREREQUISITE_GROUP = {
    "NodeInterleaving", "X2ApicMode", "CpuPhysicalAddressLimit",
    "MemoryEncryption", "GlobalMemoryIntegrity", "IntelSgx", "IntelTxt",
}

# Settings that depend on TME-MT (MemoryEncryption=MultipleKeys) being active
TDX_DEPENDENT_GROUP = {"IntelTdx", "TmeMtTdxKeySplit", "TdxSeamLoader"}


def parse_args():
    parser = argparse.ArgumentParser(
        description="Audit Dell PowerEdge R660 BIOS settings for Intel TDX readiness"
    )
    parser.add_argument(
        "--host", "-H",
...
def audit_bios_settings(host, username, password):
    ...
    print(f"Connecting to R660 iDRAC at {host} for TDX configuration audit...")
    try:
    ...
    print("\n=== INTEL TDX BIOS SETTINGS AUDIT REPORT (R660) ===\n")
